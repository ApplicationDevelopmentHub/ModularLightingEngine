...
Sphere::Sphere(float radius, uint32_t stacks, uint32_t sectors)
    : mesh(std::make_unique<Mesh>()),stacks(stacks),sectors(sectors),radius(radius) {
...
//Shape modification using the inspector
std::uint32_t Sphere::GetStack() const {
    return stacks;
}

void Sphere::SetStack(std::uint32_t stack) {
    stacks = stack;
}

std::uint32_t Sphere::GetSectors() const {
    return sectors;
}
void Sphere::SetSectors(std::uint32_t sector) {
    sectors = sector;
}
